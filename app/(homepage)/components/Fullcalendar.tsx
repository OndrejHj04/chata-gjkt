...
export default function FullcalendarComponent({
  ...
  const calendarEventData = data.data.map((event: any) => ({
    ...
    ...(event.status.id !== 5 && {
      url: `/reservation/detail/${event.id}/info`,
    }),
  }));
      ...
      <Tooltip
        title={
          <List className="p-0">
            ...
            <ListItem className="!p-0">
              <ListItemText>
                Pokoje: {rooms.map((item: any) => item.id).join(",")}
              </ListItemText>
            </ListItem>
            <ListItem className="!p-0">
              <ListItemText>
                Vedoucí: {leader.first_name} {leader.last_name}
              </ListItemText>
            </ListItem>
            <ListItem className="!p-0">
  ...
  const handleBlocation = async () => {
    setBlockedDates({
      from_date: selectedDays[0],
      to_date: dayjs(selectedDays[1]).subtract(1, "day").format("YYYY-MM-DD"),
    });
    setSelectedDays([]);
    refresh();
  };
    ...
    <Paper className="flex w-full h-full sm:flex-row flex-col p-2">
      <div className="flex flex-col sm:mr-2 mb-2 gap-2">
        ...
        {Room.getAllRooms().map((room) => (
          <Button
            key={room.id}
            onClick={() => handleFilterRoom(room.id)}
            size="small"
          >
            <div className="normal-case text-white flex items-center">
              ...
              <Checkbox
                disableRipple
                checked={searchParams.get(room.id) === "1" ? true : false}
              />
            </div>
          </Button>
        ))}
        {role === "admin" && (
          <Tooltip title="Kliknutím nebo potažením mezi jednotlivými dny vyberete období k blokaci">
            <span>
              <Button
                variant="contained"
                fullWidth
                disabled={!selectedDays.length}
                onClick={handleBlocation}
                size="small"
              >
                Blokovat
              </Button>
      ...
      <div className="flex-1" style={{ minHeight: 450 }}>
        <FullCalendar
          ...
          height="100%"
          locale={csLocale}
          headerToolbar={{ right: "", left: "" }}
          plugins={[dayGridPlugin, interactionPlugin]}
          initialView="dayGridMonth"
          events={calendarEventData}
          eventContent={eventContentInjection}
          select={(date) => setSelectedDays([date.start, date.end])}
          unselect={() => setSelectedDays([])}
          unselectCancel=".MuiButtonBase-root"
          defaultAllDay
        />
      </div>
    </Paper>

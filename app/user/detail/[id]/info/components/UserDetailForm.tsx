  ...
  return (
    <div>
      <ChangeProfilePictureModal
        ...
        setOpen={setProfileImageModal}
        userId={userDetail.id}
      />
      <CardHeader
        className="!p-0"
        avatar={<AvatarWrapper data={{ image: userDetail.image }} size={56} />}
        title={<Typography variant="h5">{userDetail.name}</Typography>}
      ...
      <List className="w-fit">
        ...
        <ListItem disablePadding>
          <ListItemText>
            Datum narození:{" "}
            {dayjs(userDetail.birth_date).format("DD. MMMM YYYY")}
          </ListItemText>
        </ListItem>
      </List>
      <form
      ...
      >
        <Button variant="outlined" onClick={() => setProfileImageModal(true)}>
          Změnit profilový obrázek
        </Button>
        {!!userDetail.parent_id && (
          <Alert severity="info">
            Rodinný účet uživatele: {userDetail.parent_name}.
          </Alert>
        )}
        {!!userDetail.children && (
          <Alert severity="info">Správce rodinných účtů.</Alert>
        )}
        <TextField
          label="Číslo OP"
          {...register("ID_code", { required: true })}
        />
        <TextField label="Adresa" {...register("adress", { required: true })} />
        <TextField
          select
          {...register("organization")}
          defaultValue={userDetail.organization_id}
          label="Organizace"
        >
